...
##regressão logistica
```{r}
regressaoLogistica <- train(situacao_final ~ .,
                 data = treino,
                 method="glm",
                 trControl = ctrl, 
...
arvore <- rpart(situacao_final ~ ., data = treino %>% select(-nome), control = control)
prp(arvore)
...
```{r}
arvore1 <- train(situacao_final ~.,data=treino, 
                 method = "rpart",
                 trControl = ctrl,
...
```{r}
data.balanced.over <- ovun.sample(situacao_final~., data=treino,p=0.5, seed=1,method="both")$data

table(data.balanced.over$situacao_final)
```
