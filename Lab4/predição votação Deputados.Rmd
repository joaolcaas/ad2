...
```{r}
lasso <- train(votos~ ., data = df %>% select(-cargo) %>% select(-nome),
               method='lasso',
               trControl = ctrl, 
               metric='RMSE',
               tuneLength = 100, 
...
ridge_prediction <= predict(lasso,df %>% select(-cargo) %>% select(-nome) %>% select(-votos))
...
KNN <- train(votos ~., data = df %>% select(-cargo) %>% select(-nome), 
             method = "knn",
             trControl=trctrl,
             metric='RMSE',
...
knnpredict <- predict(KNN, df %>% select(-cargo) %>% select(-nome) %>% select(-votos))
